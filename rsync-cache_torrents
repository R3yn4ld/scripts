#!/bin/bash
# This script gets active torrents list from transmission and tells rsync to copy them to cache drive
...
# settings
        ...
        #Rsync path
        RS_STORAGE_PATH="/mnt/user0/data/"
        RS_CACHE_PATH="/mnt/cache/data/"
...
# for each torrent
        ...
        # for each file
                ...
                do
                        ...
                        STORAGE_FILE="${RS_STORAGE_PATH}${FILE_TO_CACHE}"
                        CACHE_FILE="${RS_CACHE_PATH}${FILE_TO_CACHE}"
        # cache it!
                        STORAGE_SIZE=$(du -s "${STORAGE_FILE}" | awk '{print $1}')
                        CACHE_SIZE=$(du -s "${DESTFILE}" | awk '{print $1}')
                        if [[ $STORAGE_SIZE -eq $CACHE_SIZE ]]
                        then
                                echo -e "\t\t-> File cached, nothing to do"
                        else
                                echo -e "\t\t=> File not cached, caching full file"
                                rsync -avhP "${STORAGE_FILE}" "${CACHE_FILE}"
        echo ""
                        fi
                done
        echo ""
